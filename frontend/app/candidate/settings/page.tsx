    ...
    <div className="p-4 md:p-8 lg:p-10 max-w-3xl mx-auto">
      ...
      <div className="space-y-5">
        ...
        <section className="bg-white dark:bg-[#2c2c2e] rounded-[1.5rem] shadow-[0_12px_40px_-12px_rgba(25,28,30,0.08)] border border-surface-container p-5 md:p-6">
          <h2 className="text-base font-bold text-on-surface mb-4 flex items-center gap-2"><Icon name="account_circle" className="text-primary" />Account</h2>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="flex items-center gap-2 px-4 py-3 bg-surface-container-low rounded-2xl text-on-surface-variant font-medium">
                <Icon name="mail" className="text-outline text-lg" />
            ...
            <div>
              ...
              <div className="flex items-center gap-2 px-4 py-3 bg-surface-container-low rounded-2xl text-on-surface font-medium capitalize">
                <Icon name={role === 'recruiter' ? 'work' : 'person'} className="text-primary text-lg" />
                {role}
            ...
            <div>
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <input value={fullName} onChange={(e) => setFullName(e.target.value)} className="flex-1 px-4 py-3 bg-surface-container-low border-2 border-transparent rounded-2xl focus:border-primary focus:bg-white dark:focus:bg-white/10 transition-all text-on-surface font-medium outline-none" placeholder="Your name" />
                <button onClick={saveName} disabled={savingName} className="px-5 py-3 rounded-2xl premium-gradient text-white font-bold text-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-all disabled:opacity-60">
                  <Icon name={savingName ? 'hourglass_top' : 'check'} className="text-base" />{savingName ? 'Saving...' : 'Save'}
                </button>
              </div>
        ...
        {/* Change password */}
        <section className="bg-white dark:bg-[#2c2c2e] rounded-[1.5rem] shadow-[0_12px_40px_-12px_rgba(25,28,30,0.08)] border border-surface-container p-5 md:p-6">
          <h2 className="text-base font-bold text-on-surface mb-4 flex items-center gap-2"><Icon name="lock" className="text-primary" />Change Password</h2>
          <div className="space-y-3">
            <div className="relative">
              <input type={showPw ? 'text' : 'password'} value={pw} onChange={(e) => setPw(e.target.value)} placeholder="New password" className="w-full px-4 py-3 pr-12 bg-surface-container-low border-2 border-transparent rounded-2xl focus:border-primary focus:bg-white dark:focus:bg-white/10 transition-all text-on-surface font-medium outline-none" />
              <button onClick={() => setShowPw((v) => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-outline hover:text-on-surface"><Icon name={showPw ? 'visibility_off' : 'visibility'} className="text-lg" /></button>
            </div>
            ...
            {pwMsg && (
              <div className={`flex items-start gap-2 rounded-xl px-4 py-3 ${pwMsg.type === 'ok' ? 'bg-green-50 dark:bg-green-500/15 border border-green-200 dark:border-white/10' : 'bg-red-50 dark:bg-red-500/15 border border-red-200 dark:border-white/10'}`}>
                <Icon name={pwMsg.type === 'ok' ? 'check_circle' : 'error'} className={pwMsg.type === 'ok' ? 'text-green-500' : 'text-red-500'} />
            ...
            <button onClick={changePassword} disabled={savingPw} className="px-5 py-3 rounded-2xl premium-gradient text-white font-bold text-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-all disabled:opacity-60">
              <Icon name={savingPw ? 'hourglass_top' : 'lock_reset'} className="text-base" />{savingPw ? 'Updating...' : 'Update password'}
            </button>
          </div>
        </section>
        ...
        {/* Session */}
        <section className="bg-white dark:bg-[#2c2c2e] rounded-[1.5rem] shadow-[0_12px_40px_-12px_rgba(25,28,30,0.08)] border border-surface-container p-5 md:p-6">
          <h2 className="text-base font-bold text-on-surface mb-1 flex items-center gap-2"><Icon name="logout" className="text-primary" />Session</h2>
          ...
          <button onClick={signOut} disabled={signingOut} className="px-5 py-3 rounded-2xl bg-red-50 dark:bg-red-500/15 text-red-600 dark:text-red-300 font-bold text-sm flex items-center justify-center gap-2 hover:bg-red-100 dark:hover:bg-red-500/25 transition-all disabled:opacity-60">
            <Icon name="logout" className="text-base" />{signingOut ? 'Signing out...' : 'Sign out'}
          </button>
        </section>
      </div>
    </div>
