...
export default function RecruiterLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="bg-background text-on-background min-h-screen">
      ...
      <aside className={`fixed left-0 top-0 h-screen w-64 bg-white flex flex-col p-5 z-[60] md:hidden shadow-2xl transition-transform duration-300 ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <header className={`fixed top-0 right-0 left-0 flex items-center justify-between px-4 md:px-8 h-16 bg-white/80 backdrop-blur-md border-b border-slate-200/70 z-50 transition-all duration-300 ${collapsed ? 'md:left-20' : 'md:left-64'}`}>
        <div className="flex items-center gap-3">
          <div className="flex md:hidden items-center gap-2">
            <button onClick={() => setMobileMenuOpen(true)} className="p-2 -ml-2 text-slate-600 hover:bg-slate-100 rounded-lg transition-colors" aria-label="Open menu">
