...
const scoreColor = (s: number) => (s >= 75 ? 'text-green-600' : s >= 45 ? 'text-amber-600' : 'text-slate-500')
  ...
  return (
    <div className="p-4 md:p-8 lg:p-10 max-w-5xl mx-auto">
      ...
      <div className="flex bg-surface-container-low rounded-2xl p-1 mb-5 w-fit">
        <button onClick={() => setTab('screen')} className={`px-4 py-2.5 rounded-xl text-sm font-bold transition-all ${tab === 'screen' ? 'bg-white shadow text-primary' : 'text-on-surface-variant'}`}>Screening</button>
        <button onClick={() => setTab('copilot')} className={`px-4 py-2.5 rounded-xl text-sm font-bold flex items-center gap-1.5 transition-all ${tab === 'copilot' ? 'bg-white shadow text-primary' : 'text-on-surface-variant'}`}><span className="material-symbols-outlined text-base">smart_toy</span>Copilot</button>
      </div>

      {tab === 'copilot' ? <Copilot /> : (
      <>
      {/* Controls */}
      <div className="bg-white p-3 md:p-4 rounded-[1.5rem] shadow-[0_12px_40px_-12px_rgba(25,28,30,0.08)] border border-surface-container mb-5 flex flex-col sm:flex-row gap-3 sm:items-center">
        <div className="relative flex-1">
          ...
          <select value={jobId} onChange={(e) => setJobId(e.target.value)} className="w-full appearance-none pl-12 pr-10 py-3 bg-surface-container-low border-2 border-transparent rounded-2xl focus:border-primary focus:bg-white transition-all text-on-surface font-medium outline-none cursor-pointer" disabled={jobs.length === 0}>
        ...
        <div className="flex bg-surface-container-low rounded-2xl p-1">
          <button onClick={() => setMode('rank')} className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${mode === 'rank' ? 'bg-white shadow text-primary' : 'text-on-surface-variant'}`}>Rank Applicants</button>
          <button onClick={() => setMode('kit')} className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${mode === 'kit' ? 'bg-white shadow text-primary' : 'text-on-surface-variant'}`}>Interview Kit</button>
        </div>
      ...
      {error && (
        <div className="mb-5 flex items-start gap-2 rounded-2xl bg-red-50 border border-red-200 px-4 py-3">
        ...
        !screen ? (
          ...
          <div className="space-y-3">
            ...
            {screen.ranked.map((r, i) => {
              ...
              return (
                <div key={r.application_id} className="bg-white p-4 md:p-5 rounded-[1.5rem] shadow-[0_12px_40px_-12px_rgba(25,28,30,0.08)] border border-surface-container">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className={`px-2.5 py-1 rounded-full text-[11px] font-bold flex-shrink-0 ${REC_STYLE[r.recommendation] || 'bg-slate-100 text-slate-500'}`}>{r.recommendation}</span>
                      </div>
                      ...
                      <div className="grid sm:grid-cols-2 gap-3 mt-3">
                        {r.strengths.length > 0 && (
                          <div>
                            <p className="text-[10px] font-black text-green-700 uppercase tracking-widest mb-1">Strengths</p>
                        ...
                        {r.concerns.length > 0 && (
                          <div>
                            <p className="text-[10px] font-black text-amber-700 uppercase tracking-widest mb-1">Concerns</p>
        ...
        !kit ? (
          ...
          <div className="space-y-4">
            ...
            {kit.categories.map((c, i) => (
              <div key={i} className="bg-white p-5 rounded-[1.5rem] shadow-[0_12px_40px_-12px_rgba(25,28,30,0.08)] border border-surface-container">
                ...
                <div className="space-y-3">
                  {c.questions.map((q, j) => (
                    <div key={j} className="pl-3 border-l-2 border-primary/30">
                      ...
                      <p className="text-xs text-on-surface-variant mt-0.5"><span className="font-bold text-green-700">Look for:</span> {q.look_for}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {kit.red_flags.length > 0 && (
              <div className="bg-amber-50 border border-amber-200 p-5 rounded-[1.5rem]">
                <h3 className="text-sm font-black text-amber-900 mb-2 flex items-center gap-2"><span className="material-symbols-outlined text-amber-600 text-lg">flag</span>Red Flags</h3>
                <ul className="space-y-1">{kit.red_flags.map((r, i) => <li key={i} className="text-sm text-amber-900 flex items-start gap-1.5"><span className="material-symbols-outlined text-amber-500 text-sm mt-0.5">warning</span>{r}</li>)}</ul>
              </div>
            )}
            <div className="bg-white p-4 rounded-[1.5rem] border border-surface-container flex items-start gap-2"><span className="material-symbols-outlined text-primary">lightbulb</span><p className="text-sm text-on-surface"><span className="font-bold">Tip:</span> {kit.closing_tip}</p></div>
          </div>
        )
      )}
      </>
