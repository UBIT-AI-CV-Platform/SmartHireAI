...
export default function RecruiterError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-white/5 flex items-center justify-center p-6">
      <div className="bg-white dark:bg-[#2c2c2e] rounded-2xl shadow-sm border border-slate-200 dark:border-white/10 p-8 max-w-md w-full text-center">
        <Icon name="error" className="text-4xl text-red-400 mb-3 block" solid />
        ...
        <div className="flex gap-3 justify-center">
          ...
          <Link href="/recruiter" className="px-5 py-2.5 bg-slate-100 dark:bg-white/10 text-slate-700 dark:text-slate-300 text-sm font-semibold rounded-xl hover:bg-slate-200 dark:hover:bg-white/10 transition-colors">
            Go home
          </Link>
        </div>
      </div>
