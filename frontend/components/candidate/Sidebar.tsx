  ...
  const avatar = (
    <div className="h-9 w-9 rounded-full bg-indigo-100 dark:bg-indigo-500/15 flex items-center justify-center flex-shrink-0 overflow-hidden">
      {userPhoto ? (
        ...
        <Icon name="person" className="text-indigo-700 dark:text-indigo-300 text-lg" solid />
      )}
    </div>
  )
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className={`flex items-center mb-6 px-1 ${collapsed ? 'flex-col gap-2' : 'gap-3'}`}>
        <BrandLogo size={collapsed ? 34 : 30} showText={!collapsed} subtitle="Candidate Portal" className={collapsed ? '' : 'flex-1 min-w-0'} />
        {onToggleCollapse && (
      ...
      <nav className="flex-1 space-y-0.5 overflow-y-auto -mr-2 pr-2">
        {candidateNavLinks.map((link) => {
          ...
          return (
            ...
            >
              <Icon name={link.icon} className="text-[19px]" solid={isActive} />
